...
- (void)viewDidLoad {
    ...
    UIBarButtonItem *rightItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"select"] style:UIBarButtonItemStyleBordered target:self action:@selector(showMenu)];
    rightItem.tintColor=[UIColor whiteColor];
    self.navigationItem.rightBarButtonItem =rightItem;
    [self.tableView.header beginRefreshing];
    sheet = [[UIActionSheet alloc]initWithTitle:@"请选择" delegate:self cancelButtonTitle:@"取消" destructiveButtonTitle:nil otherButtonTitles:@"csdn",@"cnblogs", @"oschina",@"51cto",@"iteye",nil];
    [sheet setActionSheetStyle:UIActionSheetStyleBlackOpaque];
    titleArr= [[NSArray alloc] initWithObjects:@"csdn",@"cnblogs", @"oschina",@"51cto",@"iteye", nil];
}
-(void)showMenu{
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex{
    if(buttonIndex!=5) {
        type = buttonIndex+1;
        self.title=[titleArr objectAtIndex:buttonIndex];
        [self.tableView.header beginRefreshing];
    }
}
...
-(void)more{
    pageno++;
    [MainService getBlogs:pageno andType:2 andSuccess:^(NSMutableArray *result) {
